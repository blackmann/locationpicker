...
class RichSuggestion extends StatelessWidget {
  ...
  final Widget Function(
    ...
    VoidCallback onTap,
  )? autoCompleteItemBuilder;
  RichSuggestion(this.autoCompleteItem, this.onTap,
      {this.autoCompleteItemBuilder});

  @override
  Widget build(BuildContext context) {
    return autoCompleteItemBuilder != null
        ? autoCompleteItemBuilder!(autoCompleteItem, onTap)
        : Material(
            child: InkWell(
              child: Container(
                padding: EdgeInsets.symmetric(horizontal: 24, vertical: 16),
                child:
                    RichText(text: TextSpan(children: getStyledTexts(context))),
              ),
              onTap: onTap,
            ),
          );
